...
{
	CreateConstBuffer(dr, &ar.sceneParamsCB, gFrameCount * sizeof(AlignedSceneConstantBuffer));
#if NAME_D3D_RESOURCES
...
#if NAME_D3D_RESOURCES
	rt.rayGenProg.pRootSignature->SetName(L"DXR RGS Local Root Signature");
#endif
}
...
static void CreateGlobalRootSignature(DeviceResources& dr, AppResources& ar)
{
	...
	// Create the root signature
	CreateRootSignature(dr, rootDesc, &ar.globalRootSignature);
#if NAME_D3D_RESOURCES
	ar.globalRootSignature->SetName(L"Global Signature");
#endif
}

static void CreateRTPipelineStateObject(DeviceResources& dr, AppResources& ar, RayTracingResources& rt)
{
	// Need 10 subobjects:
	// 1 for RGS program
	// 1 for Miss program
	...
	//Global root Signature
	{
		...
		globalRootSig.pDesc = &ar.globalRootSignature;
...
void Application::Init(UINT width, UINT height, BOOL vsync, std::string meshFilePath)
{
	...
	CreateGlobalRootSignature(dr, ar);
	CreateRTPipelineStateObject(dr, ar, rt);
